...
func _handle_move_input():
	...
	if Input.is_action_pressed("ui_right"):
		motion = current_gravity.tangent() * -1
	if Input.is_action_pressed("ui_left"):
		motion = current_gravity.tangent() 
	if Input.is_action_pressed("ui_up"):
		motion = current_gravity
		motion= motion.normalized() * speed *100
		return
	if Input.is_action_pressed("ui_down"):
		motion = current_gravity*-1
		motion= motion.normalized() * speed *100
		return
	if Input.is_action_just_pressed("ui_jump"):
		#should inform physics_process to perform a impulse oposite to gravity
		motion=(current_gravity+motion.normalized())*speed*10
		return
	#if Input.is_action_pressed("ui_down"):
	#	motion.y += 1
